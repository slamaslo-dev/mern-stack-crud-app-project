...
const KidDetail = ({ selected, handleFormView, handleDeleteKid }) => {
  ...
  return (
    <section>
      ...
      <button onClick={() => handleFormView(selected)}>Edit Kid</button>
      <button onClick={() => handleDeleteKid(selected._id)}>Delete Kid</button>

      <div>
        <h3>Basic Info</h3>
        <p>Birth Date: {new Date(selected.birthDate).toLocaleDateString()}</p>
      </div>
